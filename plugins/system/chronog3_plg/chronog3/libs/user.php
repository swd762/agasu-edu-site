...
class User{
	...
	public static function getInstance($user_id = 0){
		...
		if(empty($instances[$user_id])){
			...
			if(\G3\Globals::get('app')){
				//$user = '\G3\L\Users\User'.strtoupper(\G3\Globals::get('app'));
				$user = \G3\Globals::getClass('user');
	...
	public function get($key, $default = null){
		...
		$value = \GApp3::session()->get('user.'.$key, $default);
	...
	public function set($key, $value){
		...
		\GApp3::session()->set('user.'.$key, $value);
	}
