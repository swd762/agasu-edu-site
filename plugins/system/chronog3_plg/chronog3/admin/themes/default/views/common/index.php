	...
	<div class="ui">
		<?php foreach($toolbar as $button): ?>
			...
			<?php
				...
				if($type == 'submit'){
					$type = 'button';
					$extra = 'data-selections="1" data-message="'.(!empty($button['message']) ? $button['message'] : rl3('Please make a selection')).'"';
				}
			?>
					...
					<?php if($text == '*'): ?>
						<td class="collapsing">
							<div class="ui checkbox selector">
								<input type="checkbox" class="hidden" name="gcb[]" value="<?php echo \G3\L\Arr::getVal($row, $path, 0); ?>">
								<label></label>
							</div>
						</td>
					<?php else: ?>
						...
						<?php else: ?>
							<td class="<?php echo $class; ?>"><?php echo \G3\L\Arr::getVal($row, $path); ?></td>
						<?php endif; ?>
					<?php endif; ?>
				<?php endforeach; ?>
			</tr>
