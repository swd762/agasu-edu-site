...
<?php
	...
	$tvout = '';
	$class = '';
	$link = !empty($link) ? $link : r3('index.php?ext='.$this->extension.'&cont='.$this->controller.'&act='.$act.'&gcb='.$id.'&fld='.$field.'&val='.(int)!(bool)$value);
	$actions = '{}';
	...
	if(!empty($dynamic) OR !empty($states)){
		$tvout = '&tvout=view';
		$class = 'G3-dynamic2 '.$act.'-'.$field.'-'.$id;
...
<?php if(!empty($states)): ?>
	<?php foreach($states as $v => $color): ?>
		<?php
			...
			$link = r3('index.php?ext='.$this->extension.'&cont='.$this->controller.'&act='.$act.'&gcb='.$id.'&fld='.$field.'&val='.(int)!(bool)$v.$tvout);
			$actions = json_encode([
				'click' => [
					[
						'act' => 'ajax',
